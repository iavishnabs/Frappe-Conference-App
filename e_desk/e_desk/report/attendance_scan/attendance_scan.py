...
def execute(filters=None):
	...
	if filters.get("from_date") and filters.get("to_date"):	
		food_scan = frappe.get_all("Attendance List", filters={"datetime": ["between", [filters.get("from_date"), filters.get("to_date")]]}, fields=['parent', 'datetime'],group_by='datetime')
		...
		for i in participant:
			...
			party[i.name]['scan_dates'].extend([b.datetime.strftime('%d-%m-%y %H:%M:%S') for b in frappe.get_all("Attendance List", filters={"datetime": ["between", [filters.get("from_date"), filters.get("to_date")]],'parent':i.name}, fields=['parent', 'datetime'],group_by='datetime')])  
